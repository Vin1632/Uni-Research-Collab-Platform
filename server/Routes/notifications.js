...
router.post('/notification/respond', async (req, res) => {
  const { collaboratorId, projectId, action } = req.body;

  if (!collaboratorId || !projectId || !action) {
    return res.status(400).json({ error: 'Missing parameters' });
  }

  try {
    const result = await respond_to_invitation(collaboratorId, projectId, action);
